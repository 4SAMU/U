...
export async function loadWalletKey(keypairFile: any) {
  const fs = require("fs");
...
async function main() {
  ...
  const myKeypair = await loadWalletKey("wallet.json");

  console.log(myKeypair);
  if (myKeypair.secretKey) {
    try {
      const mint = new web3.PublicKey(
        "4zQ8c4q7tFPJgZRMjvtdxWrDXZSAu9XZmWMG8mELPXBV"
      );
      ...
      const dataV2 = {
        name: "Zingara",
        symbol: "ZN",
        uri: "https://raw.githubusercontent.com/4SAMU/Ciesers-cipher/main/zingara.jpeg",
        // we don't need that
